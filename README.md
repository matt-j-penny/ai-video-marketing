...
| [video-production/full-edit](skills/video-production/full-edit/) | Remotion version: raw talking-head clip to a finished, graphics-covered video in one pass (`/full-edit`): ffmpeg rough cut, then a Remotion composition of full-screen, overlay, and split motion graphics, run with your saved preferences and a first-time setup that checks ffmpeg, Node, Deepgram and Brave keys. |
| [capcut/full-edit](skills/capcut/full-edit/) | CapCut version (`/full-edit-capcut`): same pipeline and preferences, but builds the edit as a CapCut draft with motion graphics placed on overlay tracks. |
| [capcut/faceless-video](skills/capcut/faceless-video/) | Topic to finished faceless video in CapCut (`/faceless-video`): researched, de-AI'd script, Inworld voiceover, Deepgram word timings, a shot list cut on key nouns, style-consistent AI images (Topview or Kie.ai), Ken Burns zooms, and word-highlighted native captions. Saved preferences for length, aspect ratio, images vs video, and cut speed; runs all at once or step by step. |

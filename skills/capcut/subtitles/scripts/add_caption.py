...
def build_styles(base_tm_style, text, base_color, highlight, highlight_color, highlight_font):
...
def main():
    ...
    base_style = base_content["styles"][0] if base_content.get("styles") else {"size": args.font_size, "strokes": []}
    ...
    def mutate(d, folder):
        ...
        content["styles"] = build_styles(base_style, args.text, args.color, args.highlight,
                                          args.highlight_color, args.highlight_font)
        ...
        if args.words:
            ...
            tm["words"] = even_words(args.text, args.dur)
        ...
        seg["source_timerange"] = {"start": 0, "duration": round(args.dur * 1e6)}
        seg["target_timerange"] = {"start": round(args.at * 1e6), "duration": round(args.dur * 1e6)}
        if args.render_index is not None:
            seg["render_index"] = args.render_index

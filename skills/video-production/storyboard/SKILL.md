...
Use a capture/compare loop: render representative frames, check them against the approved storyboard images, and correct layout/scale/spacing/color/timing before calling a scene done. Then assemble scenes into a seamless Remotion composition, and render the final video only when asked.
